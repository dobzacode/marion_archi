  ...
  return (
    <header className={cn(sectionVariants({ className, size, intent }))} {...props}>
      ...
      <Nav
        linkSize="small"
        intent={textColor}
        className="slideInFromTop hidden max-h-screen w-full items-center justify-between gap-sub-large bg-primary1 px-small opacity-0 animation-delay-[1.6s] max-laptop-large:pl-sub-large laptop:flex laptop-large:gap-large laptop-large:px-large"
      ></Nav>
      <MobileNav
        linkSize="small"
        intent={textColor}
        size={size}
        className="slideInFromTop relative z-[60] flex h-[1.125rem]  w-full items-center justify-between bg-primary1 px-small animation-delay-[1.6s] mobile-large:gap-large mobile-large:px-medium laptop:hidden"
      ></MobileNav>
    </header>
  );
}
